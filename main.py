...
def create_embed(data: dict) -> disnake.Embed:
    ...
    embed.add_field(name=f"{data['event']} на планете {data['location']}", value=f"На сервере {len(a2s.players(ip))}/{a2s.info(ip).max_players}", inline=False)
...
async def bot_set_status() -> None:
    if client.flag:
        data = get_data()
        if data["event"] != "База" and client.events is False:
            client.events = True
            channel = client.get_channel(int(os.getenv("CHANNEL_ID")))
            embed = create_embed(data)
            await channel.send(embed=embed)
        elif data["event"] == "База" and client.events is True:
            client.events = False
